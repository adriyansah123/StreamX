...
if (is_null($keyword)) {
    $daftarcomic = mysqli_query($conn, "SELECT * FROM `comic` ORDER BY `year` DESC");
} else {
    $daftarcomic = mysqli_query($conn, "SELECT * FROM `comic` WHERE `title` LIKE '%$keyword%' ORDER BY `year` DESC");
}
?>
        ...
        <div id="halamanweb">
            <div style="margin-top: 100px;">
                <div class="row justify-content-center">
                    <?php foreach ($daftarcomic as $list): ?>
                        <div class="col-lg-2 col-sm-6 mb-3">
                            <div class="card" style="height: 450px; position: relative;">
                                <div class="ribbon">
                                    <?= $list['type']; ?>
                                </div>
                                <img src="admin/<?= $list['cover']; ?>" class="card-img-top" alt="..." width=""
                                    height="200px">
                                <div class="card-body">
                                    <h5 class="card-title"><?= $list['title']; ?> (<?= $list['year']; ?>)</h5>
                                    <p class="card-text">
                                        <?php
                                        // Membuat array nama
                                        $genre = $list['genre'];
                                    ...
                                    <a class="moviepage btn btn-primary" href="reader.php?id=<?= $list['id']; ?>" data-id="<?= $list['id']; ?>">Baca Komik</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
